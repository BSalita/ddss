...
const vector<string> backendList =
{
  "cpu",
  "compare"
};

const vector<string> compareList =
{
  "fast",
  "hybrid",
  "exact",
  ...
  const char base[])
{
  ...
  cout <<
    ...
    "                   (Default: 0 meaning that DDS decides)\n" <<
    "\n" <<
    "-b, --backend b    One of: cpu, compare.\n" <<
    "                   (Default: cpu)\n" <<
    "\n" <<
    "-r, --random-deals n  Generate n random deals and solve\n" <<
    "                   through selected backend flow.\n" <<
    ...
    "                   (Default: 42)\n" <<
    "\n" <<
    "-c, --compare-target t  In compare mode: fast, hybrid,\n" <<
    "                   exact, all.\n" <<
    "                   (Default: all)\n" <<
    "\n" <<
    "-q, --confidence-threshold x  Hybrid fallback threshold\n" <<
    "                   in [0.0, 1.0].\n" <<
    "                   (Default: 0.95)\n" <<
    "\n" <<
    "-g, --report-dir p Directory for compare reports.\n" <<
...
void SetDefaults()
{
  ...
  options.memoryMB = 0;
  options.backend = DTEST_BACKEND_CPU;
  options.compareTarget = DTEST_COMPARE_ALL;
...
void PrintOptions()
{
  ...
  cout << setw(12) << "memory" << setw(12) <<  
    options.memoryMB << " MB\n";
  cout << setw(12) << "backend" << setw(12) <<
    backendList[options.backend] << "\n";
  ...
  cout << setw(12) << "seed" << setw(12) <<
    options.randomSeed << "\n";
  cout << setw(12) << "compare" << setw(12) <<
    compareList[options.compareTarget] << "\n";
  cout << setw(12) << "confidence" << setw(12) <<
  ...
  while ((c = GetNextArgToken(argc, argv)) > 0)
  {
    switch(c)
    {
      ...
      case 'b':
        ...
        for (unsigned i = 0; i < DTEST_BACKEND_SIZE && ! matchFlag; i++)
        {
          string s = backendList[i];
          transform(s.begin(), s.end(), s.begin(), ::tolower);
          if (stmp == s)
          {
            m = static_cast<int>(i);
        ...
        if (matchFlag)
          options.backend = static_cast<DdsBackend>(m);
        else
        {
          cout << "Backend '" << optarg << "' not found\n";
          nextToken -= 2;
          errFlag = true;
        }
        break;
      ...
      case 'c':
        ...
        for (unsigned i = 0; i < DTEST_COMPARE_SIZE && ! matchFlag; i++)
        {
          string s = compareList[i];
          transform(s.begin(), s.end(), s.begin(), ::tolower);
          if (stmp == s)
          {
            m = static_cast<int>(i);
        ...
        if (matchFlag)
          options.compareTarget = static_cast<CompareTarget>(m);
        else
        {
          cout << "Compare target '" << optarg << "' not found\n";
          nextToken -= 2;
          errFlag = true;
        }
        break;
      ...
      case 'q':
        d = strtod(optarg, &ctmp);
        if (d < 0.0 || d > 1.0)
        {
          cout << "Confidence threshold must be in [0.0, 1.0]\n\n";
          nextToken -= 2;
          errFlag = true;
        }
        options.confidenceThreshold = d;
        break;
